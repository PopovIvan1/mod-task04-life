...
namespace cli_life
{
    ...
    {
        public bool IsAlive;
        public int combination_number = 0;
        ...
        static void Main(string[] args)
        {
            ...
            for (int x = 0; x < board.Columns; x++)
                for (int y = 0; y < board.Rows; y++)
                {
                    if (board.Cells[x,y].IsAlive)
                    {
                        ...
                        int F2 = 0;
                        for (int i = 0; i < 8; i++)
                            if (board.Cells[x, y].neighbors[i].combination_number > 0 && board.Cells[x, y].neighbors[i].combination_number < min)
                            {
                                F2 = 1;
                                min = board.Cells[x, y].neighbors[i].combination_number;
                            }
                        if (F2 == 0)
                        {
                            count_combination++;
                            board.Cells[x, y].combination_number = count_combination;
                        }
                        else board.Cells[x, y].combination_number = min;
                    }
                }
            ...
            for (int i = 0; i < count_combination; i++)
            {
                ...
                for (int x = 0; x < board.Columns; x++)
                    for (int y = 0; y < board.Rows; y++)
                    {
                        if (board.Cells[x, y].combination_number == i + 1) 
                        {
                            arr1.Add(x);
                            arr2.Add(y);
                        }
            ...
            Console.WriteLine("\nCombinations: " + count_combination.ToString());
